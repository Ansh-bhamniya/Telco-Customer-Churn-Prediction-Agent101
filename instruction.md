...
The situation:
we have customer in the directory (`/workspace/data/train.csv`) we want to analyze which customers are likely to cancel their plans so the company can take prevention action. the model will tested on a hidden test.csv to measure real performance.
...
Constraints:
1. Use only available libraries (torch, Pandas scikit-learn, xgboost, etc.)
